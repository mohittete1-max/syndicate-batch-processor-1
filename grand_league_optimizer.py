...
# ==========================================
# 1. LIVE MATCH ENVIRONMENT & TOSS CONTROL
# ==========================================
ACTIVE_VENUE = "Sportpark Westvliet, The Hague"

# --- POST-TOSS CONTROL CENTER ---
# Update these two variables immediately after the toss!
TOSS_WINNER = "RTD"       # Enter the team acronym that won the toss (e.g., "RTD" or "ADF")
TOSS_DECISION = "Bowl"    # Enter "Bat" or "Bowl"

# Automatically figure out who is bowling first and batting first for the modifiers
...
ENTRY_FEE = 49
ENTRY_MODE = "Grand League (Post-Toss Bias Active)"
ENGINE_NAME = "Universal Syndicate Protocol v3.6 (Apex Ceiling + Toss Bias)"
CSV_OUTPUT_NAME = "Grand_League_Optimal_XI.csv"
DASHBOARD_NAME = "grand_league_dashboard.html"
...
print(f"Toss Result: {TOSS_WINNER} won and chose to {TOSS_DECISION}.")
print(f"Match Script: {BATTING_FIRST} Batting First | {BOWLING_FIRST} Bowling First\n")
...
# Ensure we only pick players actually confirmed in the starting XI!
df = raw_df[raw_df['status'] == 'Playing'].copy()

# ==========================================
# 4. HIGH-OCTANE TOSS-BIASED EXPECTED VALUE
# ==========================================
projected_ev = []
for _, row in df.iterrows():
    ...
    if role in ['bat', 'wk']:
        ...
        ev += (lambda_r * 0.9)  # Boundary & strike rate acceleration overlay
    elif role == 'bowl':
        lambda_w = cr_factor * 1.90 * form_weight
        ev = calculate_bowling_ev(max(lambda_w, 0.1))
        ev += 18.0  # Economy & maiden over bonuses
    elif role == 'ar':
        ...
        ev += (lambda_r * 0.7) + 15.0  # Dual-threat boundary + fielding spike overlay

    # --- TOSS BIAS MODIFIERS ---
    # 1. Boost 1st Innings Bowlers (Death Bowling Wicket Potential)
    if team == BOWLING_FIRST and role in ['bowl', 'ar']:
        ev *= 1.15  
    # 2. Boost 1st Innings Batters (Target Setting Potential)
    ...
    ev += 10.0  # Field activity baseline
...
def solve_lineup(script_name, focus_constraint_func=None):
    ...
    role_limits = {'wk': (1, 4), 'bat': (1, 6), 'ar': (1, 6), 'bowl': (1, 6)}
    ...
    prob += lpSum(player_vars[pid] for pid in df[df['role'] == 'ar']['player_id']) >= 5
...
final_team, is_anchored = solve_lineup("Grand_League_Spike_Toss")
...
fig = px.bar(
    ...
    color='role',
    orientation='h',
    title=f"Grand League 1000+ Toss Bias XI ({ACTIVE_VENUE})",
    hover_data=['team', 'credits', 'Multiplier', 'projected_points'],
    color_discrete_map={'ar': '#a855f7', 'bowl': '#10b981', 'bat': '#3b82f6', 'wk': '#ef4444'}
)
fig.update_layout(template="plotly_dark", xaxis_title="Post-Toss Projected EV", yaxis_title="Player Name")
