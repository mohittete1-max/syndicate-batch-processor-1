"""
==============================================================================
SYNDICATE OS - RAPIDAPI SCHEDULE FETCHER & LOCAL CACHE FALLBACK
==============================================================================
"""
...
def fetch_endpoint(endpoint):
    ...
    try:
        ...
        print(f"  [DEBUG] Endpoint '/{endpoint}' returned status code: {response.status_code}")
...
def load_from_local_cache():
    if os.path.exists(MATCH_CODES_FILE):
        print(f"📁 [FALLBACK] Loading fixtures from local cache file ('{os.path.basename(MATCH_CODES_FILE)}') due to API quota limits.")
        try:
...
def parse_and_index_fixtures():
    ...
    print(f"\n[SYSTEM] Querying multi-endpoint RapidAPI pools: {current_date_str}")
    ...
    # If API requests failed or returned empty due to 429 quota limits, fall back to local cache
    if not extracted_fixtures:
        extracted_fixtures = load_from_local_cache()

    if extracted_fixtures:
        # Save fresh copy if retrieved successfully
        if any(payloads):
            with open(MATCH_CODES_FILE, "w", encoding="utf-8") as f:
                json.dump(extracted_fixtures, f, indent=4)
                
        print(f"\n--- MASTER FIXTURE CATALOG ({len(extracted_fixtures)} fixtures loaded) ---")
        ...
        print("[INFO] No fixtures found from API and no local cache file available.")
